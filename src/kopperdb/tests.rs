...
use rocket::{Config, time::Instant};
...
impl TestClient {
    fn new() -> Self {
        TestClient { db_path: String::from(DB_PATH), seg_size: SEGMENT_SIZE, clean: true }
    }

    fn set_seg_size(mut self, seg_size: u64) -> Self { self.seg_size = seg_size; self }  
    fn dont_clean(mut self) -> Self { self.clean = false; self }

    fn build(self) -> rocket::local::blocking::Client {
        if self.clean  {
            match std::fs::remove_dir_all(DB_PATH) { _ => () }
        }
        ...
        let client = rocket::local::blocking::Client::untracked(
            rocket::custom(&config)
                ...
                .manage(create_kopper(&self.db_path, self.seg_size).expect("Can't create opper")) 
                .manage(create_stats())
        )
...
fn database_recovers_after_dying() {
    ...
    let client = TestClient::new().dont_clean().build();
    for i in key_values {
...
fn database_does_not_grow_forever() {
    ...
    // Send 1000 identical requests
    let (key, value) = random_key_value();
    let url = format!("/write/{}/{}", key, value);
    for _ in 0..1000 {
        client.get(&url).dispatch();
    }

    // Verify that database is much smaller (less than half) than 1000 x (key + value + 2)
    let all_entries_together_size = 1000 * (10 + 10 + 2) / 2;
